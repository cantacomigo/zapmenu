...
export const ManagerDashboard: React.FC<ManagerDashboardProps> = ({ restaurantId, onLogout }) => {
  ...
  const NavButton: React.FC<{ id: TabType, icon: any, label: string }> = ({ id, icon: Icon, label }) => (
    <button 
        onClick={() => setTab(id)} 
        className={`w-full flex items-center px-4 py-3 rounded-xl text-xs font-bold transition-all relative group ${tab === id ? 'bg-slate-900 text-white shadow-lg shadow-slate-900/10' : 'text-slate-500 hover:bg-slate-50'}`}
    >
        <Icon className={`w-4 h-4 mr-3 ${tab === id ? 'text-emerald-400' : 'text-slate-400 group-hover:text-slate-600'}`} />
        {label}
        {tab === id && <div className="absolute right-3 w-1 h-1 rounded-full bg-emerald-400"></div>}
    </button>
  );

  return (
    <div className="flex h-screen bg-slate-50/50 font-sans overflow-hidden">
      {/* Sidebar - Reduced to w-72 */}
        ...
        <div className="p-6 pb-4">
             <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                 ...
                 <div className="truncate">
                     ...
                     <Badge color="bg-emerald-100 text-emerald-700" className="text-[10px] py-0">Online</Badge>
                 </div>
             </div>
        </div>
        ...
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-20 px-6 py-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="w-8 h-8 bg-gradient-to-br from-emerald-400 to-teal-600 rounded-xl shadow-md"></div>
                </div>
            </div>
        </header>
                        ...
                        <div className="space-y-6">
                            <Card className="p-6 bg-slate-900 text-white relative overflow-hidden group">
                                ...
                                <div className="space-y-3 relative z-10">
                                    <Button size="sm" className="w-full bg-emerald-500 border-none py-3" onClick={() => {
                                        navigator.clipboard.writeText(menuUrl);
                                        toast.success("Copiado!");
                                    }}><Copy className="w-3.5 h-3.5 mr-2" /> Copiar Link</Button>
                                    
                                    <a href={menuUrl} target="_blank" className="flex items-center justify-center gap-2 py-2 text-[10px] font-black text-emerald-400 uppercase tracking-widest">
